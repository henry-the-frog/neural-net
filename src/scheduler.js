...
// ===== Function-based schedulers (simpler API) =====

/**
 * Step decay: multiply LR by factor every N epochs
 */
export function stepDecayFn(initialLR, factor = 0.5, stepSize = 10) {
 ...
 */
export function exponentialDecayFn(initialLR, decay = 0.99) {
 ...
 */
export function cosineAnnealingFn(initialLR, totalEpochs, minLR = 0) {
  return (epoch) => {
...
/**
 * Warmup function: linear warmup then constant or decay
 */
export function warmupFn(targetLR, warmupEpochs, afterWarmup = null) {
  return (epoch) => {
    if (epoch < warmupEpochs) return targetLR * (epoch + 1) / warmupEpochs;
    if (afterWarmup) return afterWarmup(epoch - warmupEpochs);
    return targetLR;
  };
}
 ...
 */
export function warmupCosineFn(initialLR, warmupEpochs, totalEpochs, minLR = 0) {
  const cosine = cosineAnnealingFn(initialLR, totalEpochs - warmupEpochs, minLR);
  return warmupFn(initialLR, warmupEpochs, cosine);
}

/**
 * Cyclic learning rate
 */
export function cyclicLRFn(baseLR, maxLR, cycleLength = 20) {
  return (epoch) => {
...
export function reduceLROnPlateau(initialLR, factor = 0.5, patience = 5) {
  ...
  return {
    ...
    step(loss) {
      if (loss < bestLoss * 0.999) { bestLoss = loss; waitCount = 0; }
      else { waitCount++; if (waitCount >= patience) { currentLR *= factor; waitCount = 0; } }
      return currentLR;
    }
  };
}
